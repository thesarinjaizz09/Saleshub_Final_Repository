try {
    ...
    io.on("connection", async (socket) => {
        ...
        socket.on("get_user_2", async ({ authToken, password }, callback) => {
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    socket.emit("got_user_2", { password: process.env.CLIENT_SOCKET_PASSWORD, user })
                    return callback()
                } else {
            ...
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    if (team._teamPendingMembers.find(checkUser)) {
                        ...
                        socket.join(updatedTeam._id)

                        teamData = updatedTeam

                        socket.to(user._teamId).emit("team_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedTeam })
                        return callback()
                    }
        ...
        socket.on("register_user_team", async ({ authToken, teamName, teamPassword, password }, callback) => {
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    if (`"${id}"` !== JSON.stringify(user._id)) return callback('User authentication failed...')
                    if (user._teamRole !== "neutral" || user._teamId !== "") return callback('You cannot create multiple teams..')
                    ...
                    if (newTeam._teamAdmin !== id && `"${newTeam._teamAdmin}"` !== JSON.stringify(user._id)) {
                        const teamId = newTeam._id
                        const { errr } = await deleteTeam({ teamId })
                        return callback('Registration thread denied...')
                    }
                    ...
                    socket.emit("registered_user_team", { password: process.env.CLIENT_SOCKET_PASSWORD, newTeam })
        ...
        socket.on("add_member", async ({ authToken, teamId, memberName, memberEmail, memberPassword, password }, callback) => {
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    if (team._teamMembers.length >= 3) return callback("Maximum members reached...")
                    const checkUser = (user) => {
        ...
        socket.on("join_team", async ({ authToken, teamId, teamPassword, password }, callback) => {
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    socket.to(updatedUser._teamId).emit("team_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedTeam })
                    socket.join(updatedUser._teamId)
                    return callback()
                } else {
                    return callback('Access denied...')
        ...
        socket.on("leave_team", async ({ authToken, teamId, teamPassword, password }, callback) => {
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    const { id, err } = verifyToken({ authToken })
                    if (err) return callback(err);
            ...
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    if (user._wpIntegration && user._wpSessionData !== "") {
                        ...
                        if (chatGettingErr) return callback(chatGetttingErr)
        ...
        socket.on("get_chat_details", async ({ authToken, password, chatNumber }, callback) => {
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    const { id, err } = verifyToken({ authToken })
                    if (err) return callback(err);
                    ...
                    if (user._wpIntegration && user._wpSessionData !== "") {
                        ...
                        if (!user._ownChats.find(checkChat) && !user._assignedChats.find(checkChat)) return callback('Chat not found...')
                        ...
                        if (user._ownChats.find(checkChat)) {
                            ...
                            return callback()
                        } else if (user._assignedChats.find(checkChat)) {
                            const getChatDetails = (chat) => {
                                if (chat.chatNumber === chatNumber) {
                                    return chat
                                }
                            }
                            const foundChat = user._assignedChats.find(getChatDetails);
            ...
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    if (user._wpIntegration && user._wpSessionData !== "") {
                        ...
                        if (user._ownChats.find(checkChat)) return callback('Chat already exists...')

                        else if (!user._ownChats.find(checkChat)) {
        ...
        socket.on("delete_chat", async ({ authToken, password, chatNumber }, callback) => {
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    const { id, err } = verifyToken({ authToken })
                    if (err) return callback(err);
                    ...
                    if (user._wpIntegration && user._wpSessionData !== "") {
                        ...
                        const chat = user._ownChats.find(getChatDetails)
                        if (!user._ownChats.find(checkChat) && !user._assignedChats.find(checkChat)) return callback('Chat authentication failed...')

                        if (user._ownChats.find(checkChat)) {
                            const { deletedChat, chatDeletionErr } = await deleteChat({ chatId: chat.chatId })
                            if (chatDeletionErr) return callback(chatDeletionErr)

                            var ownChats = user._ownChats
                            ...
                            if (deletedChat._chatHandlerId !== "") {
                                ...
                                const userUpdates = {
                                    _assignedChats: [...ownChats],
                                }
                                const { updatedUser, errrrr } = await updateUsers({ id: chatHandler.user._id, userUpdates })
                                if (errrrr) return callback(errrrr);

                                socket.to(updatedUser._teamId).emit("member_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser, target: updatedUser._id })
                            }
                        ...
                        if (user._assignedChats.find(checkChat)) {
                            const chat = user._assignedChats.find(getChatDetails);

                            var ownChats = user._assignedChats
                            ...
                            socket.to(updatedUser._teamId).emit("member_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser, target: updatedUser._id })
                            socket.to(updatedUser._teamId).emit("got_chat_details", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedChat, target: updatedChat._chatParent })
                        }
                    }
                }
            } catch (err) {
        ...
        socket.on("update_chat", async ({ authToken, password, chatUpdates, chatNumber }, callback) => {
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    const { id, err } = verifyToken({ authToken })
                    if (err) return callback(err);
                    ...
                    if (user._wpIntegration && user._wpSessionData !== "") {
                        ...
                        if (user._assignedChats.find(checkChat)) {
                            ...
                            if (updatedChat._chatParent !== id) {
                                const chatHandler = await getUsers({ id: updatedChat._chatParent })
                                if (chatHandler.error) return callback(chatHandler.error)

                                var chatDetails;
                                var ownChats = chatHandler.user._ownChats
                                var chatId = JSON.stringify(updatedChat._id)
                                for (let index = 0; index < ownChats.length; index++) {
                                    const element = ownChats[index];
                                    if (`"${element.chatId}"` === chatId) {
            ...
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    if (user._wpIntegration && user._wpSessionData !== "") {
                        ...
                        const chat = user._ownChats.find(checkChat)
                        if (!user._ownChats.find(checkChat)) return callback('Chat authentication failed...')
                    ...
                    if (!funnelGettingErr) {
                        ...
                        if (!existingFunnel._funnelChats.find(checkFunnelChat)) {
                            ...
                            if (user._teamRole !== 'neutral') {
                                ...
                                if (existingChat._chatParent !== id || existingChat._) return callback('Chat authentication failed...')
                            }
                            ...
                            // console.log(existingChat._chatMessages[existingChat._chatMessages.length].from)
                            const funnelUpdates = {
                                _funnelChats: [{
        ...
        socket.on("assign_chat_to_member", async ({ authToken, password, memberId, chatId }, callback) => {
            try {
                console.log("Got request to assign")
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    const { id, err } = verifyToken({ authToken })
                    if (err) return callback(err);
                    ...
                    if (user._teamRole === 'admin' && user._teamId !== "") {
                        ...
                        if (teamMember.user._assignedChats.find(checkChat)) return callback('Chat has been already assigned to the member...')
                        ...
                        if (existingChat._chatHandlerId !== "") {
                            ...
                            var ownChats = prevTeamMember.user._assignedChats
                            ...
                            const userUpdates2 = {
                                _assignedChats: [{
                                    ...
                                    chatLabel: existingChat._chatLabel
                                }, ...teamMember.user._assignedChats]
                            }
                            ...
                            socket.to(user._teamId).emit("member_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser, target: updatedUser._id })
                            socket.to(user._teamId).emit("member_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser: updatedUser2.updatedUser, target: updatedUser2.updatedUser._id })
                            socket.emit("got_chat_details", { password: process.env.CLIENT_SOCKET_PASSWORD, chat: updatedChat })
                            return callback()
        ...
        socket.on("integrate_whatsapp", async ({ authToken, password }, callback) => {
            try {
                console.log("Integrating whatsapp in middle server...")
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    const { id, err } = verifyToken({ authToken })
                    if (err) return callback(err);
                    ...
                    if (!user._wpIntegration && user._wpSessionData === "") {
                        ...
                        client.on('ready', async () => {
                            console.log("Neutral client ready...")
                            const { error } = await getUsers({ id });
                            if (error) return callback(error);
                        ...
                        client.on("message", async (message) => {
                            if (message._data.id.participant) {
                                console.log('Message Ignored due to group chat...')
                            }
                            else if (message.type === 'document') {
                                console.log('Message ignored due to document....')
                            }
                            else if (message._data.type === 'chat') {
                                console.log("Chat Message processing in authentcation...");
                                var chatId;
                                var returnedUser;
                                ...
                                if (user._wpIntegration && user._wpSessionData !== "") {
                                    ...
                                    if (user._ownChats.find(checkChat)) {
                                        console.log("Chat found in the user...")
                                        const getChatDetails = (chat) => {
                                            if (chat.chatNumber === clientNumber) {
                                                return chat;
                                            }
                                        }
                                        chatId = user._ownChats.find(getChatDetails).chatId
                                        ...
                                        var ownChats = user._ownChats
                                        for (let index = 0; index < ownChats.length; index++) {
                                            const element = user._ownChats[index];
                                            if (element.chatId === chatId) {
                                                chatDetails = ownChats.splice(index, 1)
                                                break;
                                            }
                                        ...
                                        if (updatedChat._chatHandlerId !== "" && updatedChat._chatHandlerId !== id) {
                                            ...
                                            var ownChats = chatHandler.user._assignedChats
                                            for (let index = 0; index < ownChats.length; index++) {
                                                const element = chatHandler.user._assignedChats[index];
                                                if (element.chatId === chatId) {
                                                    chatDetails = ownChats.splice(index, 1)
                                                    break;
                                                }
                                            ...
                                            const userUpdates = {
                                                _assignedChats: [orgChat, ...ownChats]
                                            }
                                            const { updatedUser, errrrr } = await updateUsers({ id: chatHandler.user._id, userUpdates })
                                            if (errrrr) return callback(errrrr);

                                            socket.to(user._teamId).emit("member_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser, target: updatedUser._id })
                                            socket.to(user._teamId).emit("whatsapp_message_for_member", { password: process.env.CLIENT_SOCKET_PASSWORD, chat: updatedChat, target: updatedUser._id })
                                        }

                                        isChatAlreadyUpdated = true
                                    } else if (!user._ownChats.find(checkChat) && !user._assignedChats.find(checkChat)) {
                                    ...
                                    if (user._assignedChats.find(checkChat)) {
                                        ...
                                        chatId = user._assignedChats.find(getChatDetails).chatId
                                        ...
                                        var ownChats = user._assignedChats
                                        for (let index = 0; index < ownChats.length; index++) {
                                            const element = user._assignedChats[index];
                                            if (element.chatId === chatId) {
                                                chatDetails = ownChats.splice(index, 1)
                                                break;
                                            }
                                        ...
                                        var chatDetails2;
                                        var ownChats2 = teamAdmin.user._ownChats
                                        for (let index = 0; index < ownChats2.length; index++) {
                                            const element = teamAdmin.user._ownChats[index];
                                            if (element.chatId === chatId) {
                                        ...
                                        socket.to(user._teamId).emit("member_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser, target: updatedTeamAdmin.updatedUser._id })
                                        socket.to(user._teamId).emit("whatsapp_message_for_member", { password: process.env.CLIENT_SOCKET_PASSWORD, chat: updatedChat, target: updatedTeamAdmin.updatedUser._id })
                                    }
                                    ...
                                    if (returnedUser.user._assignedChats.find(checkChat)) {
                                    ...
                                    } else if (returnedUser.user._ownChats.find(checkChat)) {
                                        if (existingChat._chatParent !== id) return callback('Chat authentication failed...')
                                    }
                                ...
                                if (message._data.type === 'image') {
                                    ...
                                    if (user._wpIntegration && user._wpSessionData !== "") {
                                        ...
                                        if (user._ownChats.find(checkChat)) {
                                            ...
                                            scoket.emit("user_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser })
                                        ...
                                        if (user._assignedChats.find(checkChat)) {
                                            ...
                                            const userUpdates2 = {
                                                _assignedChats: [orgChat2, ...ownChats2]
                                            }
                                            const updatedTeamAdmin = await updateUsers({ id: teamAdmin.user._id, userUpdates: userUpdates2 })
                                            if (updatedTeamAdmin.errrrr) return callback(updatedTeamAdmin.errrrr);

                                            socket.to(user._teamId).emit("member_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser: updatedTeamAdmin.updatedUser, target: updatedTeamAdmin.updatedUser._id })
                                            socket.to(user._teamId).emit("whatsapp_message_for_member", { password: process.env.CLIENT_SOCKET_PASSWORD, chat: updatedChat, target: updatedTeamAdmin.updatedUser._id })
                                        }
                                        ...
                                        if (!user._assignedChats.find(checkChat)) {
                                            ...
                                            socket.to(user._teamId).emit("whatsapp_message_from_assigned", { password: process.env.CLIENT_SOCKET_PASSWORD, chat: existingChat, target: userTeam._teamAdmin })
                                        }
        ...
        socket.on("authenticate_whatsapp", async ({ authToken, password }, callback) => {
            try {
                ...
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    if (`"${id}"` !== JSON.stringify(user._id)) return callback("User authentication failed...")
                    if (!user._wpIntegration || user._wpSessionData === "") return callback("Whatsapp not integrated...")

                    const sessionId = user._wpSessionData;
            ...
            try {
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    if (user._wpIntegration && user._wpSessionData !== "" && permission) {
                        ...
                        if (!isImage) {
                            if (!user._ownChats.find(checkChat) && !user._assignedChats.find(checkChat)) {
                            ...
                            if (user._assignedChats.find(checkChat)) {
                                ...
                                const { updatedUser, errrr } = await updateUsers({ id, userUpdates })
                                if (errrr) return callback(errrr);
                                ...
                                socket.to(user._teamId).emit("member_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser: updatedTeamAdmin.updatedUser, target: updatedTeamAdmin.updatedUser._id })
                                socket.to(user._teamId).emit("sent_message_for_member", { password: process.env.CLIENT_SOCKET_PASSWORD, chat: updatedChat, target: updatedTeamAdmin.updatedUser._id })
                                return callback()
                            }
                        } else if (isImage) {
                            if (!user._ownChats.find(checkChat) && !user._assignedChats.find(checkChat)) {
                            ...
                            if (user._assignedChats.find(checkChat)) {
                                ...
                                socket.emit("sent_message", { password: process.env.CLIENT_SOCKET_PASSWORD, chat: updatedChat })
                                socket.emit("user_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser })
                                socket.to(user._teamId).emit("member_updated", { password: process.env.CLIENT_SOCKET_PASSWORD, updatedUser: updatedTeamAdmin, target: userTeam._teamAdmin })
                                socket.to(user._teamId).emit("sent_message_for_member", { password: process.env.CLIENT_SOCKET_PASSWORD, chat: updatedChat, target: userTeam._teamAdmin })
                            }
                        }
                ...
                if (password === process.env.SERVER_SOCKET_PASSWORD) {
                    ...
                    if (user._wpIntegration && user._wpSessionData !== "" && permission) {
                        ...
                        if (user._assignedChats.find(checkChat)) {
                            ...
                            socket.emit("sent_message", { password: process.env.CLIENT_SOCKET_PASSWORD, chat: updatedChat })
                            socket.to(user._teamId).emit("sent_message_for_member", { password: process.env.CLIENT_SOCKET_PASSWORD, chat: updatedChat, target: team._teamAdmin })
                        }
                    }
                } else {
                    return callback("Access denied...")
